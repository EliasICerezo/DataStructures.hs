...
weights [] = D.empty
weights x = pesos x D.empty

pesos:: Ord a => [a] -> D.Dictionary a Int -> D.Dictionary a Int
pesos [] d = d
pesos (x:xs) d= pesos xs (D.updateOrInsert x (+1) 1 d)
...
huffmanLeaves s = hojas  (D.keysValues (weights s)) PQ.empty

hojas:: [(Char,Int)]->PQ.PQueue (WLeafTree Char)->PQ.PQueue (WLeafTree Char)
hojas [] p = p
hojas (x:xs) p = hojas xs (PQ.enqueue  (WLeaf (fst x) (snd x)) p )
...
huffmanTree s =if( D.size (weights s) < 2 ) then error"La cadena tiene menos de 2 simbolos" else   arbolh (huffmanLeaves s)

arbolh::PQ.PQueue (WLeafTree Char)->WLeafTree Char
arbolh p
      |PQ.size p==1 = PQ.first p
      |otherwise = arbolh (PQ.enqueue (merge a1 a2) (PQ.dequeue (PQ.dequeue p)) )
      where
          a1= PQ.first p
...
joinDics d1 d2
      |D.isEmpty d1 = d2
      |D.isEmpty d2 = d1
      |otherwise = junta (D.keysValues d1) (D.keysValues d2) (D.empty)

junta:: Ord a => [(a,b)] -> [(a,b)] -> D.Dictionary a b-> D.Dictionary a b
junta [] [] d     = d
junta [] (y:ys) d = junta [] ys (D.insert (fst y) (snd y) d)
junta l [] d      = junta [] l d
junta (x:xs) (y:ys) d
          |(fst x) < (fst y) = junta xs (y:ys) (D.insert (fst x ) (snd x) d)
          |otherwise = junta (x:xs) ys (D.insert (fst y) (snd y) d)
...
prefixWith e d= prefija e (D.keysValues d) D.empty



prefija::  Ord a => b -> [(a,[b])] -> D.Dictionary a [b] -> D.Dictionary a [b]
prefija e [] d = d
prefija e (x:xs) d = prefija e xs (D.insert (fst x) ([e]++snd x) d)
...
huffmanCode t = codigo t [] D.empty

codigo::WLeafTree Char->[Integer]->D.Dictionary Char [Integer]->D.Dictionary Char [Integer]
codigo (WLeaf a _) l d   = D.insert a l d
codigo (WNode lt rt _) l d = joinDics (codigo lt (l++[0]) d) (codigo rt (l++[1]) d)
