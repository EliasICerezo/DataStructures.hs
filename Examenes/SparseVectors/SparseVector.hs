...
-------------------------------------------------------------------------------
-- | Exercise c.
simplyfy:: (Eq a)=>Tree a->Tree a->Tree a
simplyfy (Unif x) (Unif y)
                |x==y = Unif x
                |otherwise = (Node (Unif x) (Unif y))
simplyfy lt rt = (Node lt rt)
...
get:: Int->Vector a->a
get n (V s t)
        |n<1 || n>s = error"Idx out of bounds"
        |isUnif t = value t
        |n<=(div s 2) = get n (V (div s 2) (left t))
        |n>(div s 2) = get (n-(div s 2)) (V (div s 2) (right t))
...
set i x (V s t)
          |i<1 || i>s = error"Idx out of bounds"
          |isUnif t = (V s (Unif x))
          |i <= (div s 2) = (V s (simplyfy (cambia i (div s 2) x (left t) ) (right t) ) )
          |i >(div s 2) = (V s (simplyfy (left t) (cambia (i - (div s 2)) (div s 2) x (right t)) ))

cambia::(Eq a)=> Int->Int->a->Tree a->Tree a
cambia n s x (Unif y) = (Unif x)
cambia n s x (Node lt rt)
                  |n<=(div s 2) =  simplyfy (cambia n (div s 2) x lt)  rt
                  |n>(div s 2) =   simplyfy  lt  (cambia (n-(div s 2)) (div s 2) x rt)
-------------------------------------------------------------------------------
-- | Exercise f.
